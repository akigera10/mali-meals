...
export default async function PackingSlipsPage({
  ...
  searchParams: Promise<{ order?: string; day?: string; week?: string; date?: string }>
}) {
  ...
  if (resolvedSearchParams.order) {
    const { data } = await (supabase.from('orders') as any)
      ...
      .eq('id', resolvedSearchParams.order)
    rawOrders = data || []
  } else if (resolvedSearchParams.date) {
    const { data } = await (supabase.from('orders') as any)
      .select('id, order_ref, customer_name, customer_phone, delivery_zone, delivery_day, delivery_date, delivery_window, delivery_slot, address_building, address_street, address_apartment, address_landmark, payment_status, notes')
      .eq('delivery_date', resolvedSearchParams.date)
      .order('delivery_zone', { ascending: true })
    rawOrders = data || []
  } else if (resolvedSearchParams.day && resolvedSearchParams.week) {
    const weekStart = new Date(decodeURIComponent(resolvedSearchParams.week))
    const weekEnd = new Date(weekStart)
    weekEnd.setDate(weekEnd.getDate() + 7)
    const { data } = await (supabase.from('orders') as any)
      .select('id, order_ref, customer_name, customer_phone, delivery_zone, delivery_day, delivery_date, delivery_window, delivery_slot, address_building, address_street, address_apartment, address_landmark, payment_status, notes')
      .eq('delivery_day', resolvedSearchParams.day)
  ...
  let standaloneAddonsData: any[] = []

  if (orderIds.length > 0) {
    const [{ data: items }, { data: specials }, { data: standaloneAddons }] = await Promise.all([
    ...
    standaloneAddonsData = standaloneAddons || []
  }
  ...
  const slips: SlipOrder[] = rawOrders.map((o: any) => {
    ...
    for (const addon of standaloneAddonsData.filter((a: any) => a.order_id === o.id)) {
      const name = addon.protein_addons?.name ?? addon.addon_name ?? 'Unknown'
      const existing = addons.find(a => a.name === name)
      if (existing) existing.quantity += addon.quantity
      else addons.push({ name, quantity: addon.quantity })
    }
  ...
  const backLink = resolvedSearchParams.order
    ? `/admin/orders/${resolvedSearchParams.order}`
    : '/admin/deliveries'
  ...
  if (resolvedSearchParams.order) {
    ...
    const d = new Date(resolvedSearchParams.date + 'T12:00:00')
    const dateLabel = d.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' })
    title = `Packing slips — ${dateLabel}`
  } else {
    const dayLabel = resolvedSearchParams.day === 'monday' ? 'Monday' : 'Sunday'
